...
void MemoryManager::init(uint32_t memHigh, uint32_t physicalAllocationStart){
...
void MemoryManager::invalidate(uint32_t virAddr){
    asm volatile("invlpg %0" :: "m"(virAddr));
    // asm volatile("invlpg (%0)" ::"r" (virAddr) : "memory");
}

void MemoryManager::init_physical_memory(uint32_t memLow, uint32_t memHigh){
...
uint32_t MemoryManager::alloc_page_frame(){    
...
void MemoryManager::map_page(uint32_t vir_addr, uint32_t phys_addr, uint32_t flags){
    ...
    if(!(page_dir[page_dir_idx] & PAGE_FLAG_PRESENT)){
        uint32_t ptr_page_addr = alloc_page_frame();
    ...
    if(previous_page_dir!=0){
        ...
        if(previous_page_dir != initial_page_directory){
            change_page_dir(previous_page_dir);
        }
    }
}


uint32_t * MemoryManager::get_current_page_dir(){
    uint32_t page_dir;
...
void MemoryManager::change_page_dir(uint32_t * pd){
    pd = (uint32_t*) (((uint32_t)pd)-KERNEL_START);
    asm volatile("mov %0, %%eax \n mov %%eax, %%cr3 \n" :: "m"(pd));
    
}

void MemoryManager::sync_page_dirs(){
...
uint32_t MemoryManager::unmap_page(uint32_t vir_addr) {
    uint32_t page_dir_idx = vir_addr >> 22;
...
void MemoryManager::free_page_frame(uint32_t phys_addr) {
