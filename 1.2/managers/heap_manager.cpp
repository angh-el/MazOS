...
void change_heap_size(int new_size){
    ...
    if(new_page>old_page){
        ...
        for(int i =0; i< delta; i++){
            uint32_t physica_mem_location = MemoryManager::alloc_page_frame();
            MemoryManager::map_page(KERNEL_MALLOC+old_page *0x1000+i*0x1000, physica_mem_location, PAGE_FLAG_WRITE);
        }
    }
    // printf("yassssssss\n");
    heap_size=new_size;
...
void kfree(void *ptr) {
    ...
    uint32_t phys_addr = MemoryManager::unmap_page(addr); // Unmap and get physical address
    if (phys_addr) {
        MemoryManager::free_page_frame(phys_addr); // Mark the physical frame as free
    }
}