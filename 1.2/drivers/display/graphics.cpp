...
uint8_t Graphics::get_color_index(uint8_t r, uint8_t g, uint8_t b) {
...
void Graphics::init() {
    ...
    static unsigned char mode13h_registers[61] = {
        // MISC
        0x63,
        // SEQ
...
void Graphics::draw_pixel(int x, int y, unsigned char r, unsigned char g, unsigned char b) {
...
void Graphics::draw_line(int x1, int y1, int x2, int y2, unsigned char color) {
    int dx = x2 - x1;
    int dy = y2 - y1;
    int sx = (x1 < x2) ? 1 : -1;
    int sy = (y1 < y2) ? 1 : -1;
...
void Graphics::draw_diagonal_line() {
    for (int i = 0; i < 100; i++) {
        draw_pixel(i, i, 0, 0, 100);  // White diagonal line
    }
}

void Graphics::clear_screen(char color) {
...
void Graphics::quit() {
    ...
    Display::set_cursor(Display::get_screen_offset(50, 50));
