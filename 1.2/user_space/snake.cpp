...
void Snake::draw_game() {
    ...
    // Draw the snake
    for (int i = 0; i < snake_length; i++) {
        Display::print_char('O', snake[i].x, snake[i].y, GREEN_ON_GREEN);
    }

    // Draw the apple
    Display::print_char('X', apple.x, apple.y, RED_ON_RED);
    // printf("%d", snake[0].x);

    // Set the cursor to the bottom-right corner to avoid overwriting the game
    Display::set_cursor(Display::get_screen_offset(GAME_WIDTH - 1, GAME_HEIGHT - 1));
...
void Snake::move_snake() {
    ...
    if (snake[0].x < 0 || snake[0].x >= GAME_WIDTH || snake[0].y < 0 || snake[0].y >= GAME_HEIGHT) {
        game_over = 1;
        Display::set_cursor(Display::get_screen_offset(3, 10));
        printf("Game Over");
    }
    ...
    for (int i = 1; i < snake_length; i++) {
        if (snake[0].x == snake[i].x && snake[0].y == snake[i].y) {
            game_over = 1;
            Display::set_cursor(Display::get_screen_offset(20, 20));
            printf("Game Over");
            break;
        }
    }
...
void Snake::start_snake_game() {
    ...
    Keyboard::setCurrentMode(Mode::MODE_SNAKE);
